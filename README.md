...
从 AMLL 导入时，如果取回的 TTML 是 AMLL 格式，模块会先自动转换为 Apple Music 格式再填入编辑框：移除 `<metadata>` 和 `<div>` 上的 `xmlns=""`（该属性会让整个歌词子树脱离 TTML 命名空间，使 Apple Music 解析不出任何歌词行），并在存在逐字 `<span>` 时为根节点补上 `itunes:timing="Word"`。转换只改写标签，歌词文本和逐字之间的空格按原样保留；已是 Apple Music 格式的文件不会被改动。
