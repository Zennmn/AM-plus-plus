...
自定义歌词按 Apple Music ID 与歌曲绑定。开启“自动实时补全”后，播放过程中检测到原生歌词不是逐字时间轴或外语逐字歌词缺少翻译、且没有可用手动歌词时，模块可能请求候选歌词；关闭该开关则不会在播放过程中请求歌词服务。设置页中的手动导入仍由用户主动触发。
...
- 关闭“自动实时补全”时，模块不会在播放过程中自动请求歌词服务；开启后仅对检测到的非逐字或外语无翻译原生歌词、且没有可用手动歌词的歌曲执行候选查询。
